...
class User(Base):
    ...
    Authorization is as simple as possible. Security is not an issue.

    The only requirement is that simulations are uniquely mapped to users. 
    This is achieved by providing a single user with a single api-key.
    ...
    Initially, the developer will assign api-keys and create users
    """
    __tablename__ = "users"
